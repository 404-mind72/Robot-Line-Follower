...
int threshold = 500; // Default, overridden by calibration
bool calibrated = false;

const int buttonPin = 8; // Push button for calibration (INPUT_PULLUP expected)
...
void setup() {
  ...
  Serial.println("Tekan tombol kalibrasi atau kirim 'c' dari Serial untuk kalibrasi.");
}
...
void loop() {
  ...
  // Interpretasi: LOW = garis hitam, HIGH = putih
  bool s1 = isHitam(0);
  bool s2 = isHitam(1);
  bool s3 = isHitam(2);
  bool s4 = isHitam(3);
  ...
  else if (s1)                putarKiri();
  else if (s5)                putarKanan();
  else                        berhenti();
...
bool isHitam(int index) {
  int midpoint = (sensorMin[index] + sensorMax[index]) / 2;
  return sensorRaw[index] < midpoint;
}

// --- Calibration Logic ---
void kalibrasiSensor() {
  Serial.println("🔧 Kalibrasi dimulai... Gerakkan robot ke atas garis dan latar!");
